...
import { addExpense, editExpense } from "../../../api/expense";

const EditExpenseInstance = ({
  getExpenseList,
  setOpenEditExpense,
  setExpenseInstance,
  expenseInstance,
  ...
  const [type, setType] = useState(expenseInstance.type);
  const [amount, setAmount] = useState(expenseInstance.amount);
  const [description, setDescription] = useState(expenseInstance.description);
  ...
  useEffect(() => {
    ...
    const now = new Date(expenseInstance.date * 1000);
    let date =
      now.getFullYear() +
      "-" +
  ...
  const editExpenseFormHandler = async (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      const { data, error } = await editExpense({
      ...
      <div>
        <Container fluid className="main_content_container mx-auto">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <h3 className="main_content_heading">Edit Expense</h3>
            <CrossButton
              onClick={() => setOpenEditExpense((o) => !o)}
            ></CrossButton>
          </div>
          <Form
            noValidate
            validated={validated}
            onSubmit={editExpenseFormHandler}
          ...
          >
            <Stack gap={1}>
              ...
              <Row className="pt-9 mb-1 justify-content-end">
                <Col sm="auto">
                  <GeneralButton
                    onClickEvent={() => setOpenEditExpense((o) => !o)}
                    className="me-1"
                    value="Cancel"
                    color="#505050"
                    variant="outlined"
...
export default EditExpenseInstance;
